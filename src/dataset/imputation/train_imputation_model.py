...
class Discretizer:
    ...
    def fit(self, arr):
        ...
        for i, feat_idx in enumerate(self.cont_feats_idx):
            ...
            # 31July: ensure the column is float type
            col = col.astype(float)  # ensure the column is float type
    ...
    def transform(self, arr):
        ...
        # 31July: ensure the column is float type
        for i, feat_idx in enumerate(self.cont_feats_idx):
            col = arr[:, feat_idx].astype(float)  # ensure float for isnan
            mask = ~np.isnan(col)
            arr[:, feat_idx][mask] = (
                np.digitize(col[mask], self.bins[i], right=True) - 1
            )
    ...
    def inverse_transform(self, arr):
        ...
        # for i, feat_idx in enumerate(self.cont_feats_idx):
        #     # 31July: ensure the column is float type
        #     arr[:, feat_idx] = arr[:, feat_idx].astype(float)
        ...
        # 31July: ensure the column is float type
        for i, feat_idx in enumerate(self.cont_feats_idx):
            col = arr[:, feat_idx].astype(float)  # ensure numeric type
            mask = ~np.isnan(col)
            arr[:, feat_idx][mask] = self.representative_values[i][col[mask].astype(int)]
        return arr
...
class EM:
    ...
    def predict(self, arr, method="argmax", use_best=False) -> np.array:
        ...
        if isinstance(p_x_given_h, list):
            p_x_given_h = np.array(p_x_given_h)
    ...
    def save_model(self, path):
        ...
        # Save model parameters using np.savez
        np.savez(
            path,
            p_h=self.p_h,
            p_x_given_h=np.array([arr for arr in self.p_x_given_h], dtype=object),
            H=self.num_latent_states,
            n_iter=self.n_iter,
            num_categories=self.num_categories,
            tol=self.tol,
            discretizer_cont_feats_idx=np.array(self.discretizer.cont_feats_idx),
            discretizer_nbins=np.array(self.discretizer.n_bins),
            discretizer_bins=np.array(self.discretizer.bins, dtype=object),
            discretizer_representative_values=np.array(self.discretizer.representative_values, dtype=object),
            hist=self.hist,
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cont_feats_idx', type=int, nargs='+', default=[0, 1, 2])
    ...
    data = df[cont_feats + disc_feats].values
    ...
    em = EM(args.n_latent_states, args.n_iter, num_categories, discretizer, train_data=train_data, tol=args.tol)
    em.fit(val_data)
