...
def discretized_gaussian(
    ...
    Args:
        loc (Tensor): Location parameter tensor, shape (*, 1).
        var (Tensor): Variance parameter tensor, shape (*, 1).
        tmax (int): Maximum time.
        return_prob (bool, optional): Flag to return probabilities.
                                      Defaults to False.

    Returns:
        Tensor: Log probabilities or probabilities, shape (*, tmax).
    """
    trange = torch.arange(1, float(tmax) + 1, device=loc.device)                # [tmax]
    log_probs = -((trange - loc) ** 2) / (2 * var + 1e-8)                       # [*, tmax]
    log_probs = log_probs - torch.logsumexp(log_probs, dim=1, keepdim=True)     # [*, tmax]
...
def get_survival_function(prob: Tensor) -> np.ndarray:
    ...
    Returns:
        np.ndarray: Survival function values, shape (batch_size, tmax).
    """
    prob = prob.data.cpu().numpy()
    return np.cumsum(prob[:, ::-1], axis=1)[:, ::-1]  # reverse cumualtive sum
...
def get_mean_prediction(prob: Tensor, tmax: int) -> Tensor:
    ...
    Returns:
        Tensor: Mean prediction values, shape (batch_size,).
    """
...
def _get_censoring_mask(cens_time: Tensor, tmax: float) -> Tensor:
    r"""
    Helper function to get the mask to be used for the censored observations term in the
      likelihood. For each censored observation, the mask is 1 for all possible event
...
def classical_loss(
    ...
    logp = torch.log(surv_dists + 1e-8)
    logp_cens = logp[~delta]
    logp_uncens = logp[delta]
    loss_uncens = logp_uncens.gather(1, (uncens_time - 1).long()).sum()  # Adjust for 0-based indexing
...
def centime_loss(
    ...
    Args:
        surv_dists (Tensor): Histograms of expected survival times, shape (batch_size, tmax).
        delta (Tensor): Event indicator tensor, shape (batch_size, 1).
        time (Tensor): Time tensor, shape (batch_size, 1). Event time if uncensored,
                                    censoring time if censored.
        tmax (int): Maximum time.
    ...
    """
    time = time.view(-1)                    # [B]
    delta = delta.view(-1).type(torch.bool) # [B], cens = 0, uncens = 1
    uncens_time = time[delta].view(-1, 1)   # [n_uncens, 1]
    cens_time = time[~delta].view(-1, 1)    # [n_cens, 1]
    logp = torch.log(surv_dists + 1e-8)     # [B, tmax]

    logp_uncens = logp[delta]                                            # [n_uncens, tmax]
    loss_uncens = logp_uncens.gather(1, (uncens_time - 1).long()).sum()  # [scalar], adjust for 0-based indexing. Extracts values of logp_uncens dists at indices OF uncens_time & sums them to a scalar
    loss_uncens = -loss_uncens / len(time)                               # [scalar] # TODO is dividing by len(time) necessary?
    
    logp_cens = logp[~delta]                            # [n_cens, tmax]
    cens_mask = _get_censoring_mask(cens_time, tmax)    # [n_cens, tmax]
    time_factor = (                                     # [n_cens, tmax]
        torch.arange(0, tmax, device=surv_dists.device) # inc zero, which becomes -inf and we get 1/(t-1) instead of 1/t, as per paper
        .view(1, -1)
        .repeat(cens_time.shape[0], 1)
        .log()
    )
    logp_cens[~cens_mask] = -float("inf")  # [n_cens, tmax], ignore values in logp at times less than cens_time when computing logsumexp
    time_factor[~cens_mask] = 0            # [n_cens, tmax]
    loss_cens = -torch.logsumexp(logp_cens - time_factor, 1).sum() / len(time)  # [scalar] # TODO is dividing by len(time) necessary?
    return loss_cens, loss_uncens
...
def cox_loss(theta: Tensor, delta: Tensor, time: Tensor) -> Tensor:
    ...
    loss_cox = (
        theta.reshape(-1) - cox_logsumexp(theta.T, mask=risk_mat, dim=1)
    ) * delta.reshape(-1)
...
def deephit_likelihood(pred: Tensor, event: Tensor, time: Tensor) -> Tensor:
    ...
    """
    eps = 1e-8
    # cif = pred.cumsum(1).gather(1, time.long())  # Cumulative incidence function
    # loss_uncensored = (event * pred.gather(1, time.long())).add(eps).log().sum()
    # convert 1-based times → 0-based indices
    t_idx = (time - 1).clamp(min=0).long()
    cif = pred.cumsum(1).gather(1, t_idx)                       # Cumulative incidence
    loss_uncensored = (event * pred.gather(1, t_idx)).add(eps).log().sum()
...
def deephit_ranking(
    ...
    loss = (prior_events_mask * eta).mean() if prior_events_mask.sum() > 0 else 0.0
...
def _diff_cdf_at_time_i(pmf: Tensor, time: Tensor) -> Tensor:
    ...
    time = (time - 1).clamp(min=0).reshape(-1, 1)       # 0-based
    time = torch.zeros_like(pmf).scatter(1, time.long(), 1.0)
...
def is_valid_cox_batch(delta: Tensor, time: Tensor) -> bool:
    ...
    return bool(((risk_matrix.sum(dim=1) * delta.reshape(-1)) > 1).any())


def cox_logsumexp(
    input_tensor: Tensor, mask: Tensor, dim: int, keepdim: bool = False
) -> Tensor:
    """
